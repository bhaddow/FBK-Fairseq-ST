...
class TransformerEncoder(FairseqEncoder):
    ...
    def upgrade_state_dict(self, state_dict):
        ...
        if state_dict.get('encoder.version', torch.Tensor([2]))[0] < 2:
...
class TransformerDecoder(FairseqIncrementalDecoder):
    ...
    Args:
        args (argparse.Namespace): parsed cose volete che ci incontriamo stasera fammelo sapere per tempo, perché se andiamo a Rovereto è diffmmand-line arguments
...
@register_model_architecture('speechconvtransformer', 'speechconvtransformer')
...
@register_model_architecture('speechconvtransformer', 'speechconvtransformer_paper')
...
@register_model_architecture('speechconvtransformer', 'speechconvtransformer_big')
def speechtransformer_fbk(args):
    args.dropout = getattr(args, 'dropout', 0.3)
    args.normalization_constant = getattr(args, 'normalization_constant', 0.5)
    args.attention_dropout = getattr(args, 'attention_dropout', 0.1)
...
@register_model_architecture('speechconvtransformer', 'speechconvtransformer_giant')
def speechtransformer_fbk(args):
    args.dropout = getattr(args, 'dropout', 0.3)
    args.normalization_constant = getattr(args, 'normalization_constant', 0.5)
    args.attention_dropout = getattr(args, 'attention_dropout', 0.1)
